...
def build_background_registry() -> ToolRegistry:
    ...
    reg.register(_spec("state_signal_summary", "Conservative signal-derived state candidates", "background"), bg_tools.state_signal_summary)
...
def build_event_registry() -> ToolRegistry:
    ...
    reg.register(_spec("spike_wave_candidate_score", "Rhythmic spike-wave-like window candidate score", "event"), ev_tools.spike_wave_candidate_score)
    ...
    reg.register(_spec("event_waveform_numeric_v2", "Focused event-candidate waveform amplitude/frequency/duration measurements", "event"), ev_tools.event_waveform_numeric_v2)
    reg.register(_spec("event_spatiomorphology_v2", "Trace-safe event spatial and morphology descriptors", "event"), ev_tools.event_spatiomorphology_v2)
