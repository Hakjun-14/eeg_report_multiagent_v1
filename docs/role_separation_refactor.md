...
2. `EvidenceItem` objects are produced in `background_module.py`, `event_module.py`, and `protocol_state_context_parser.py` from those measurements.
...
5. `AtomicClaimPlan` objects are produced in `ReportSynthesizer.build_atomic_claim_plan()` from EvidenceItems.
...
- `EvidenceItem` keeps only clinical grouping semantics in new runtime outputs: `evidence item_id`, `evidence item_type`, `assertion`, quantitation, and linked `measurement_ids`.
- Legacy `EvidenceItem.provenance`, `EvidenceItem.confidence`, and `EvidenceItem.source_module` remain loadable but are no longer populated by background/event/protocol modules.
